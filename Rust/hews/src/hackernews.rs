...
pub fn top_stories(client: Arc<Client>, count: usize, tx: Arc<Mutex<Sender<Msg>>>) {
    ...
    thread::spawn(move || {
        ...
        for id in filtered {
            ...
            tx_clone.into_inner().unwrap().send(Msg::NewStory(story)).unwrap();
        }
        if loaded {
            tx.clone().into_inner().unwrap().send(Msg::Loaded).unwrap();
        }
    });
}
