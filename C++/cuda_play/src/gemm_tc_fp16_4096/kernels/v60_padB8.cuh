...
// v60: v52 mainloop schedule but with **B shared-memory padding**.
//
// Hypothesis:
// - Even with CUTLASS tensor-op swizzle (`tensorop_rm_offset`), the exact
//   leading dimension (LD) of shared-memory tiles can affect residual bank
//   conflicts for `ldmatrix`.
// - CUTLASS' `MmaBase::SharedStorage` supports per-operand padding, and many
//   Tensor Core GEMM examples pad the "N" dimension of B in shared by +8 half.
//
// This variant pads B's shared leading dimension from 256 -> 264 half.
// It keeps:
// - A shared layout identical to v52 (no padding).
// - Same TB/Warp tile and schedule (postwait overlap).
//
// Teaching notes:
// - Padding by +8 half = +16 bytes per row. That number is not arbitrary:
//   it nudges the bank mapping for 128-bit (`ldmatrix` / `uint4`) accesses,
//   which are very sensitive to the shared row stride.
// - This is *shared-only* padding: global B is still a dense row-major [K,N]
//   matrix. We only change how we lay out each 32x256 tile in shared.
// - Important: the CUTLASS TensorOpMultiplicand swizzle (`tensorop_rm_offset`)
//   depends on the leading dimension. If you pad B, you must also use the
//   padded LD in *every* place that computes shared offsets (cp.async stores,
//   and the `ldmatrix` addresses used by the compute warps).
// - Cost: B stage size increases by 32 * 8 half = 256 half = 512 bytes per stage
//   (and N_STAGES=3 => +1536B total). Usually worth it if it reduces bank
//   conflicts around `ldmatrix`.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_padB8(
    ...
    static_assert(N_STAGES == 3, "v60 is specialized for N_STAGES=3");
    ...
    // B shared padding (+8 half) on the N dimension.
    constexpr int B_LD = TILE_N + 8;          // 264 half
    constexpr int B_STAGE_ELEMS = TILE_K * B_LD; // 8448 half
    ...
    // Swapped mapping: grid.x=M tiles, grid.y=N tiles.
...
#pragma unroll
    ...
    constexpr int B_INT4_PER_ROW = TILE_N / 8; // 32 (only the real 256 columns)
...
#pragma unroll
    for (int it = 0; it < B_ITERS; ++it) {
        ...
        int const b_col = b_seg * 8; // 0..255
        b_gmem_delta[it] = b_row * kN + b_col;
        b_smem_off_bytes[it] =
            static_cast<uint32_t>(tensorop_rm_offset<64, B_LD>(b_row, b_col)
                                  << 1);
    }
...
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            ...
            b_ld_off_bytes[kk_idx][nb] = static_cast<uint32_t>(
                tensorop_rm_offset<64, B_LD>(b_row, b_col) << 1);
        }
#pragma unroll
        for (int m = 0; m < 4; ++m) {
...
#pragma unroll
    ...
    // Tail: two remaining iterations when N_STAGES=3 (same as v52).
