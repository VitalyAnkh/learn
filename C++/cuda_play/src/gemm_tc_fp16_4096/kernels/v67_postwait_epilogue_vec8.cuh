...
// v67: v52 (double-buffered A/B fragments + post-wait mainloop schedule) +
// a CUTLASS-inspired epilogue that vectorizes global stores (8 half / 16B).
//
// Compared to v66:
// - v66 uses the v65 "single-fragment" mainloop (lower regs, simpler).
// - v67 keeps v52's double-buffered fragments (potentially better overlap of
//   `ldmatrix` with HMMA), then applies the same shared-scratch + vec8 store
//   epilogue.
//
// The goal is to see whether we can gain back some mainloop throughput without
// giving up the big epilogue win of vectorized global stores.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_epilogue_vec8(
    ...
    static_assert(N_STAGES == 3, "v67 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // Tail: fall back to v52 semantics (same as v52_postwait.cuh).
    // For simplicity, we reuse v52's tail structure but keep the vec8 epilogue.
    // (This is a fixed-size benchmark kernel; tail is tiny relative to K=4096.)
    //
    // To keep this header concise, we rely on the fact that MAIN_ITERS covers
    // K_BLOCKS-(N_STAGES-1) iterations and the remaining two are handled by the
    // steady-state machinery + final iteration (same as v52 implementation).
    //
    // NOTE: This is intentionally identical to v52's end behavior by letting
    // the loop run the full MAIN_ITERS and then computing the last two blocks
    // in the same pattern as v52. We do this by reusing the last loaded slot0.

    // The v52 implementation performs two explicit tail iterations for N_STAGES=3.
    // Rather than duplicate all of it here, we simply do a final drain and rely
    // on the fact that our MAIN_ITERS loop already loaded the next kgroup0.
    //
    // For correctness, we must still finish the last two K blocks. The simplest
    // approach is to just call into the already-correct v52 tail logic, but that
    // would require linking across headers. So we keep a small explicit tail
    // here by mirroring v52's two final iterations.

    {
        // k_iter = MAIN_ITERS: compute kg0/kg1, then drain.
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;
...
#pragma unroll
        ...
        // Advance and load final kgroup0 into slot0.
        a_load_base += A_STAGE_BYTES;
        b_load_base += B_STAGE_BYTES;
...
#pragma unroll
    ...
    {
        // Final iteration: kg0+kg1 (no more prefetch).
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;

        // Load kgroup1 into slot1.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t const addr_b = b_load_base_k + b_ld_off_bytes[1][nb];
            ldmatrix_x4_trans(bReg[1][nb][0], bReg[1][nb][1], bReg[1][nb][2],
...
#pragma unroll
    ...
    // Epilogue: shared scratch + vec8 stores (same as v66).
    __syncthreads();
...
#pragma unroll
        for (int n = 0; n < 8; ++n) {
            ...
            int const idx0 = (row0 * TILE_N + col0) >> 1;
