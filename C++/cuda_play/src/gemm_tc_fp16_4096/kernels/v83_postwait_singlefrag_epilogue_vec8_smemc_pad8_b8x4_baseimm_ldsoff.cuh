...
// v83: v80 + ldmatrix addressing improvements (shared-memory base + immediate offsets).
//
// Motivation:
// - CUTLASS/CUTE reaches ~196–198 TFLOPS on this fixed 4096^3 case, while our
//   v70–v82 family is typically ~186–188 TFLOPS.
// - Disassembly shows CUTLASS often uses `LDSM ... [Rbase+imm]` addressing for
//   A-side ldmatrix loads (and `+0x2000` for B kgroup1) to reduce integer address
//   arithmetic in the hot loop.
// - Our v80 computes per-(kgroup,m) offsets into separate registers, which can
//   force extra IADD/LEA instructions around the ldmatrix region.
//
// Technique:
// - Keep the same data-movement and mainloop schedule as v80:
//   - 128x256x32 TB, 2x4 warp grid, N_STAGES=3 cp.async pipeline, postwait sync.
// - For A-side ldmatrix:
//   - Compute the per-thread base address for (kk=0, m=0).
//   - Use `ldmatrix_x4_off<0x400/0x800/0xC00>` for m=1..3 (these deltas are
//     constant for the CUTLASS TensorOpMultiplicandCrosswise<16,32> mapping).
// - For B-side ldmatrix:
//   - Compute offsets only for kgroup0 (kk=0).
//   - Load kgroup1 (kk=16) using a constant +0x2000B offset:
//       `ldmatrix_x4_trans_off<0x2000>(..., addr_kgroup0)`.
//
// Expected effect:
// - Reduce integer address arithmetic around `LDSM` (ldmatrix) loads.
// - Move instruction mix closer to CUTLASS' SASS and potentially increase
//   Tensor Core pipe utilization.
//
// Teaching notes:
// - `mma_sync_m16n8k16_row_col_f16` expects:
//   - A operand in row-major fragment layout
//   - B operand in col-major fragment layout
//   We keep B in shared as a row-major 32x256 tile, and use `ldmatrix ... trans`
//   to load it as the transposed fragment required by the MMA instruction.
// - Why the constants are nice round hex numbers:
//   - For this CUTLASS-style swizzled shared layout,
//     `m += 16 rows` corresponds to `+0x400` bytes in shared for A (`A_M_DELTA`).
//   - `kk += 16` (kgroup1) corresponds to `+0x2000` bytes in shared for B
//     (`B_KGROUP_DELTA`).
//   Having these as compile-time constants lets us use specialized helper
//   intrinsics (`ldmatrix_x4_off`, `ldmatrix_x4_trans_off`) that generate
//   `[Rbase+imm]` addressing in SASS.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_b8x4_baseimm_ldsoff(
    ...
    static_assert(N_STAGES == 3, "v83 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    constexpr uint32_t A_M_DELTA_BYTES = 0x400; // 1024B per +16 rows (see findings.md)
...
#pragma unroll
    ...
    // Tail + epilogue: copy/paste from v80, but using the new ldmatrix scheme.
    {
        // k_iter = MAIN_ITERS
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
...
#pragma unroll
    ...
    // ------------------------------------------------------------
    // Epilogue: scratch + vec8 stores (same as v70/v79/v80)
    // ------------------------------------------------------------
    __syncthreads();
