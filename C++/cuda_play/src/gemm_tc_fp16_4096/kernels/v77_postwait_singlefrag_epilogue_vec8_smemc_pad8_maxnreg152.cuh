...
// v77: v70 + `__maxnreg__(152)` (try to exactly match CUTLASS' reg count).
//
// Motivation:
// - CUTLASS reference kernel we measured uses REG=152.
// - v76 showed REG=160 can be achieved without spills but didn't improve
//   performance. This pushes further to 152 to test whether reducing register
//   pressure/power helps sustain higher clocks or improves scheduling.
//
// Risk:
// - If ptxas cannot fit within 152 registers, it will spill to local memory and
//   performance will regress sharply. We keep this version as an experiment.
template <int N_STAGES>
__global__ __maxnreg__(152) void
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_maxnreg152(
    ...
    static_assert(N_STAGES == 3, "v77 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // Tail: same structure as v70 (keep it unchanged to isolate the reg-cap effect).
    {
