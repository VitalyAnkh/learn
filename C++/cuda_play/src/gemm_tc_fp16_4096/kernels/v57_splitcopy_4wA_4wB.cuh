...
// v57: split-copy (A by 4 warps, B by 4 warps) to force warp de-phasing.
//
// Motivation:
// - v52 still leaves a noticeable gap to CUTLASS (~180 vs ~197.8 TFLOPS).
// - A key difference visible in CUTLASS SASS is that `LDGSTS` and `HMMA` are
//   mixed at finer granularity, which is only beneficial if not all warps are
//   lockstep at the copy points. Our hand interleave attempts (v53/v56) did
//   not create enough useful warp de-phasing.
// - Here we deliberately make copy work **uneven across warps**:
//     - warps 4..7 copy *all* A for the stage (4 cp.async per warp)
//     - warps 0..3 copy *all* B for the stage (8 cp.async per warp)
//   This creates a window where half the warps can be issuing HMMA while the
//   other half are still issuing LDGSTS.
//
// Notes:
// - Total bytes copied per stage is unchanged (A=8KB, B=16KB).
// - Total number of warp-level `cp.async` instructions per stage remains 48
//   (4 warps *4 + 4 warps *8).
// - We keep the same TB/warp tile and the same tensorop shared layout.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_splitcopy4wA4wB(
    ...
    static_assert(N_STAGES == 3, "v57 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // Split-copy mapping: 4 warps (128 threads) cover A, 4 warps cover B.
    constexpr int COPY_WARPS = 4;
    constexpr int COPY_THREADS = COPY_WARPS * 32; // 128
    int const copy_tid = tid & (COPY_THREADS - 1); // 0..127 (warp-local within group)
    bool const is_b_copy_warp = (warp_id < 4);
    bool const is_a_copy_warp = !is_b_copy_warp;

    constexpr int A_INT4_PER_ROW = TILE_K / 8; // 4
    constexpr int A_INT4_TOTAL = (TILE_M * TILE_K) / 8; // 512
    constexpr int A_ITERS = A_INT4_TOTAL / COPY_THREADS; // 4

    constexpr int B_INT4_PER_ROW = TILE_N / 8; // 32
    constexpr int B_INT4_TOTAL = (TILE_K * TILE_N) / 8; // 1024
    constexpr int B_ITERS = B_INT4_TOTAL / COPY_THREADS; // 8

    int a_gmem_delta[A_ITERS];
    uint32_t a_smem_off_bytes[A_ITERS];
#pragma unroll
    for (int it = 0; it < A_ITERS; ++it) {
        int const idx = copy_tid + it * COPY_THREADS;
...
#pragma unroll
    ...
    // Prologue: prefetch first (N_STAGES-1) K blocks using split-copy mapping.
#pragma unroll
...
#pragma unroll
        ...
        // MMA: kgroup0 using slot 0 (B-reuse order).
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
...
#pragma unroll
        ...
        // Split-copy: A by warps 4..7, B by warps 0..3.
        if (is_a_copy_warp) {
#pragma unroll
            for (int it = 0; it < A_ITERS; ++it) {
                cp_async_cg_L2_16B_addr(a_store_base + a_smem_off_bytes[it],
...
#pragma unroll
        ...
        // Postwait: overlap cp.async completion with kgroup1 HMMA.
        cp_async_wait_group<N_STAGES - 2>();
        __syncthreads();
        ...
        // Load next iter kgroup0 into slot 0.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t const addr_b = b_load_base + b_ld_off_bytes[0][nb];
            ldmatrix_x4_trans(bReg[0][nb][0], bReg[0][nb][1], bReg[0][nb][2],
...
#pragma unroll
    ...
    // Tail: keep v52 structure for simplicity (compute-only + drain + final).
    // Iteration MAIN_ITERS: compute, then drain last stage.
    {
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;
...
#pragma unroll
    ...
    // Final iteration: no prefetch, no barrier.
    {
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;
