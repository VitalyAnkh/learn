...
// v84: v80 + epilogue scratch stores rewritten to encourage "base + immediate"
// addressing modes (CUTLASS-like STS patterns).
//
// Motivation:
...
// - v80 already improves the mainloop copy address arithmetic; however, our
//   epilogue still emits a lot of address arithmetic for the scratch `STS`
//   stores (many different base registers instead of one base + offsets).
// - CUTLASS SASS shows epilogue `STS [Rbase.X4+0x10*lane]` patterns where the
//   n-dimension stride (8 half == 16B) becomes an immediate offset.
//
// Key change (epilogue only):
// - Keep the mainloop identical to v80.
// - Rewrite the scratch store as:
//     p0[n*4] = c[m][n][0];  p1[n*4] = c[m][n][1]
//   where `n*4` is a compile-time constant stride in `uint32_t` units.
//   This matches the natural +16B stride between n-blocks and enables the
//   compiler to emit `STS [base+0x10*...]` more often.
//
// Expected effect:
// - Reduce integer address arithmetic in the epilogue.
// - Potentially close some of the remaining gap to CUTLASS.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_b8x4_baseimm_epistore(
    ...
    static_assert(N_STAGES == 3, "v84 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    [[maybe_unused]] constexpr int B_ROW_STRIDE = kN; // in half
...
#pragma unroll
    ...
    int const row0 = warp_row * 64 + group_id;
    int const col0 = warp_col * 64 + tid_in_group * 2; // half (even)

    // Shared indexing in u32 (half2) units.
    constexpr int SMEM_C_LD_U32 = SMEM_C_LD / 2;
    constexpr int SMEM_C_ROW1_DELTA_U32 = 8 * SMEM_C_LD_U32;
    constexpr int SMEM_C_M_DELTA_U32 = 16 * SMEM_C_LD_U32;
    constexpr int SMEM_C_N_DELTA_U32 = 4; // +8 half => +16B => +4 u32

    uint32_t *base0 = smem_C_u32 + row0 * SMEM_C_LD_U32 + (col0 >> 1);
    uint32_t *base1 = base0 + SMEM_C_ROW1_DELTA_U32;

#pragma unroll
    for (int m = 0; m < 4; ++m) {
        uint32_t *p0 = base0 + m * SMEM_C_M_DELTA_U32;
        uint32_t *p1 = base1 + m * SMEM_C_M_DELTA_U32;
#pragma unroll
        for (int n = 0; n < 8; ++n) {
            p0[n * SMEM_C_N_DELTA_U32] = c[m][n][0];
            p1[n * SMEM_C_N_DELTA_U32] = c[m][n][1];
        }
    }
