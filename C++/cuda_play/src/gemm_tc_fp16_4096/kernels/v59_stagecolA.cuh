...
// v59: v52 (cutlass-style TN mainloop, 128x256x32, stages=3) but with a
// CUTLASS-like shared-memory *stage embedding* for operand A.
//
// Key idea:
// - In v49/v52, A is stored as `As[stage][M][K]` i.e. each stage is a separate
//   128x32 tile. Because each stage's base address is a large multiple of 128B,
//   the **bank mapping repeats** across stages.
// - CUTLASS' `MmaBase::SharedStorage` embeds the stage dimension into the
//   *contiguous* (K) dimension for A: A shared shape becomes [M, K*Stages].
//   This changes the bank mapping across stages and can reduce systematic
//   `ldmatrix` bank conflicts.
//
// Implementation detail (why this is cheap):
// - For `tensorop_rm_offset<32, LD>` (ElementSize=16b, Crosswise=32),
//   shifting logical `col` by +32 (i.e. +4 vectors) adds a constant +64 half
//   elements to the shared offset, independent of row/col low bits.
//   So stage `s` can be addressed as `base_offset + s * 64` (in half elements)
//   without recomputing the full swizzle.
//
// We keep:
// - B layout and pipeline identical to v52.
// - Same MMA order (B-reuse) and swapXY grid mapping.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_stagecolA(
    ...
    static_assert(N_STAGES == 3, "v59 is specialized for N_STAGES=3");
    ...
    // A shared: [M, K * N_STAGES] => 128 x 96.
    constexpr int A_LD = TILE_K * N_STAGES;            // 96
    constexpr int A_SMEM_ELEMS = TILE_M * A_LD;        // 12288 half

    // B shared keeps the classic stage-separate layout.
    ...
    // For tensorop_rm_offset<32, A_LD>:
    //   offset(row, col + 32) == offset(row, col) + 64 (half elements).
    constexpr uint32_t A_STAGE_COL_DELTA_BYTES = 64u * sizeof(__half); // 128B

    __shared__ __align__(128) __half As[A_SMEM_ELEMS];
...
#pragma unroll
    for (int it = 0; it < A_ITERS; ++it) {
        ...
        a_smem_off_bytes[it] =
            static_cast<uint32_t>(tensorop_rm_offset<32, A_LD>(a_row, a_col)
                                  << 1);
    }
...
#pragma unroll
    ...
    // Load offsets for stage 0 (stage offset is added as a constant delta).
...
#pragma unroll
        for (int m = 0; m < 4; ++m) {
            ...
            a_ld_off_bytes[kk_idx][m] = static_cast<uint32_t>(
                tensorop_rm_offset<32, A_LD>(a_row, a_col) << 1);
        }
    }

    // Prologue: prefetch first (N_STAGES-1) K blocks (stages 0 and 1).
#pragma unroll
    for (int stage = 0; stage < (N_STAGES - 1); ++stage) {
        int const k_tile = stage * TILE_K;
        uint32_t const a_stage_delta =
            static_cast<uint32_t>(stage) * A_STAGE_COL_DELTA_BYTES;
...
#pragma unroll
        for (int it = 0; it < A_ITERS; ++it) {
            cp_async_cg_L2_16B_addr(As_base + a_stage_delta + a_smem_off_bytes[it],
...
#pragma unroll
    ...
    int stage_read = 0;
    int stage_write = N_STAGES - 1; // first not-yet-loaded stage
...
#pragma unroll
    ...
    {
        uint32_t const a_load_stage_delta = 0;
#pragma unroll
        for (int m = 0; m < 4; ++m) {
            uint32_t const addr_a =
                As_base + a_load_stage_delta + a_ld_off_bytes[0][m];
    ...
    // Steady-state loop (identical structure to v52: k0 compute, prefetch, k1
    // compute, wait+barrier, advance stage, preload next k0).
    for (int k_iter = 0; k_iter < MAIN_ITERS; ++k_iter) {
        uint32_t const a_load_stage_delta =
            static_cast<uint32_t>(stage_read) * A_STAGE_COL_DELTA_BYTES;
...
#pragma unroll
        for (int m = 0; m < 4; ++m) {
            uint32_t const addr_a =
                As_base + a_load_stage_delta + a_ld_off_bytes[1][m];
...
#pragma unroll
        ...
        uint32_t const a_store_stage_delta =
            static_cast<uint32_t>(stage_write) * A_STAGE_COL_DELTA_BYTES;

#pragma unroll
        for (int it = 0; it < A_ITERS; ++it) {
            cp_async_cg_L2_16B_addr(
                As_base + a_store_stage_delta + a_smem_off_bytes[it],
...
#pragma unroll
        ...
        // Only now do we wait/sync for the next stage.
        cp_async_wait_group<N_STAGES - 2>();
        __syncthreads();

        // Advance stage indices (mod 3) and B stage pointers.
        stage_read += 1;
        stage_write += 1;
        if (stage_read == N_STAGES) {
            stage_read = 0;
        }
        if (stage_write == N_STAGES) {
            stage_write = 0;
        }
        ...
        uint32_t const a_load_stage_delta_next =
            static_cast<uint32_t>(stage_read) * A_STAGE_COL_DELTA_BYTES;
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t const addr_b = b_load_base + b_ld_off_bytes[0][nb];
...
#pragma unroll
        for (int m = 0; m < 4; ++m) {
            uint32_t const addr_a =
                As_base + a_load_stage_delta_next + a_ld_off_bytes[0][m];
            ldmatrix_x4(aReg[0][m][0], aReg[0][m][1], aReg[0][m][2],
                        aReg[0][m][3], addr_a);
        }
    }

    // Tail: two remaining iterations when N_STAGES=3 (same structure as v52).
    // At this point slot0 contains kgroup0 for k_iter = MAIN_ITERS.
    {
        // Iteration k_iter = MAIN_ITERS (e.g., 126): compute, then drain.
        uint32_t const a_load_stage_delta =
            static_cast<uint32_t>(stage_read) * A_STAGE_COL_DELTA_BYTES;
        uint32_t const b_load_base_k = b_load_base;
...
#pragma unroll
        ...
        // Drain remaining cp.async (the last stage we prefetched).
        cp_async_wait_group<0>();
        __syncthreads();

        // Advance stage indices and B pointers, then load final tile's kgroup0.
        stage_read += 1;
        if (stage_read == N_STAGES) {
            stage_read = 0;
        }
