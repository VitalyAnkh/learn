...
// v64: v52 + alternative shared-memory swizzle for operand B:
//      `tensorop_rm_offset<Crosswise=32>` instead of `Crosswise=64`.
//
// Motivation:
// - Nsight Compute shows v52 has significantly higher `Stall LG Throttle` than
//   the CUTLASS reference kernel.
// - CUTLASS' TensorOp shared-memory layouts vary the "crosswise" parameter
//   (16/32/64) depending on operand and access pattern. This variant probes
//   whether B's `Crosswise=32` reduces bank conflicts / load throttling for
//   `ldmatrix.trans` on Ada (sm_89).
//
// Design:
// - Identical to v52 in schedule and tile shape.
// - Only changes B-side `tensorop_rm_offset` Crosswise parameter.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_bcross32(
    ...
    static_assert(N_STAGES == 3, "v64 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // Tail handling identical to v52 (specialized for N_STAGES=3).
    {
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;
