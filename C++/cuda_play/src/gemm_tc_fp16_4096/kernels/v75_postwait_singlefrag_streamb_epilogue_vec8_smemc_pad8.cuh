...
// v75: v70 (singlefrag + vec8 epilogue) but **stream B fragments**.
//
// Motivation:
// - Our v70 keeps `bReg[4][4]` in registers between kgroup0 and kgroup1 loads.
//   That is 16x32-bit regs per warp just for B fragments.
// - CUTLASS/CUTE kernels on this machine use fewer registers (~152), which can
//   improve scheduling/power and help sustain higher clocks.
// - Streaming B means: load one B "nb" slice via `ldmatrix` right before its
//   HMMA group, then discard it (no persistent bReg[][] array).
//
// Expected effect:
// - ~O(10) fewer registers per thread.
// - More natural `LDSM` ↔ `HMMA` interleave (a cutlass-like trait).
//
// Notes:
// - Instruction count stays the same: still 4 B `ldmatrix.x4.trans` per kgroup,
//   just placed inside the `nb` loop.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_streamb_epilogue_vec8_smemc_pad8(
    ...
    static_assert(N_STAGES == 3, "v75 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // Initial load: A kgroup0 (kk=0) from stage 0.
#pragma unroll
    for (int m = 0; m < 4; ++m) {
...
#pragma unroll
        ...
        // Load A kgroup1 (kk=16) into the same regs (singlefrag overwrite).
#pragma unroll
        for (int m = 0; m < 4; ++m) {
        ...
        // MMA: kgroup1 (kk=16). Stream B fragments again.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t b0, b1, b2, b3;
...
#pragma unroll
        ...
        // Load next iteration's A kgroup0 (kk=0) from the new stage.
#pragma unroll
        for (int m = 0; m < 4; ++m) {
            uint32_t const addr_a = a_load_base + a_ld_off_bytes[0][m];
            ldmatrix_x4(aReg[m][0], aReg[m][1], aReg[m][2], aReg[m][3],
    ...
    // Tail (same as v70, but streaming B): keep structure clear over micro-opts.
    {
        // Compute both kgroups for the last steady-state iteration, then drain.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t b0, b1, b2, b3;
            uint32_t const addr_b = b_load_base + b_ld_off_bytes[0][nb];
...
#pragma unroll
    ...
    // -----------------------------
    // Epilogue: padded scratch (LD=264) + vec8 stores (same strategy as v70)
    // -----------------------------
    __syncthreads();
