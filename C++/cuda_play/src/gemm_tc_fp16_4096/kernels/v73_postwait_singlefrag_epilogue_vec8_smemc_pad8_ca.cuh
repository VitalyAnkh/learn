...
// v73: v70 + use `cp.async.ca` instead of `cp.async.cg`.
//
// Motivation:
// - CUTLASS often uses `cp.async.ca.shared.global[.L2::128B]` on SM80+.
// - On some chips/workloads, `.ca` can reduce effective latency (or change cache
//   behavior) compared to `.cg`, even though data ultimately lands in shared.
// - This is a small, low-risk toggle to validate on Ada (sm_89) for this
//   fixed-size GEMM.
//
// Design:
// - Mainloop + epilogue identical to v70.
// - Only change: replace `cp_async_cg_L2_16B_addr` with
//   `cp_async_ca_L2_16B_addr` in prologue + steady-state.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_ca(
    ...
    static_assert(N_STAGES == 3, "v73 is specialized for N_STAGES=3");
