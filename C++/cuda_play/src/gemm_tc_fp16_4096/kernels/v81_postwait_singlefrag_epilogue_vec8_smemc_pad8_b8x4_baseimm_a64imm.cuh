...
// v81: v80 + avoid a dedicated "A second-half" gmem pointer register by using
// a compile-time immediate offset for the +64 row copy.
//
// Motivation:
// - In v80 SASS we successfully got base+imm for B copies (both smem+gmem),
//   and for A copies in smem, but A's gmem addresses often still materialize
//   as two independent base registers.
// - The second A copy is always at (row+64, same col). In gmem, this is a fixed
//   offset of 64*kK half elements = 0x80000 bytes.
// - By using `gA + A_ROW_DELTA` directly in the cp.async call, we give the
//   compiler a chance to use `[Rbase.64+imm]` addressing and avoid:
//     - an extra pointer register
//     - an extra pointer increment per K-iter
//
// Everything else stays the same as v80:
// - 128x256x32 TB, 8 warps (2x4)
// - 3-stage cp.async pipeline, CUTLASS-style TN mainloop
// - B copy mapping: 8x4 warp-raked (4 vectors per thread, +0x80 immediates)
// - FP16 accumulate + FP16 store
// - Shared scratch + vec8 global stores (C scratch LD padded by +8 half)
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_b8x4_baseimm_a64imm(
    ...
    static_assert(N_STAGES == 3, "v81 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    constexpr uint32_t A_SMEM_ROW_DELTA_BYTES = 0x1000; // +4096B (row+64)

    int const b_row = tid >> 3;         // 0..31
    int const b_group = tid & 7;        // 0..7
    int const b_col0 = b_group * 8;     // half
    constexpr int B_VEC_DELTA = 64;     // half (+128B)
...
#pragma unroll
    for (int stage = 0; stage < (N_STAGES - 1); ++stage) {
        ...
        cp_async_cg_L2_16B_addr(a_store + A_SMEM_ROW_DELTA_BYTES, gA + A_ROW_DELTA);
    ...
    // Running gmem prefetch pointers (thread-local).
...
#pragma unroll
        ...
        // Prefetch stage k_iter + 2.
        uint32_t const a_store = a_store_base + a_smem_off0;
        uint32_t const b_store = b_store_base + b_smem_off0;

        cp_async_cg_L2_16B_addr(a_store, gA_pref);
        cp_async_cg_L2_16B_addr(a_store + A_SMEM_ROW_DELTA_BYTES, gA_pref + A_ROW_DELTA);
...
#pragma unroll
    ...
    // Tail + epilogue: identical to v80/v79 (copy-paste).
    {
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t const b0 = bReg[nb][0];
