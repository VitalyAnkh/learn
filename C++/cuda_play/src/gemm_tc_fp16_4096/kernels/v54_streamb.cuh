...
// v54: v49-style mainloop, but "stream B" fragments from shared memory.
//
// Motivation:
// - In v49/v52, we keep `bReg[2][4][4]` in registers for the whole iteration.
//   This is 2 (kgroup) * 4 (nb) * 4 (regs/ldmatrix.x4) = 32x32-bit registers
//   per warp just for B fragments.
// - CUTLASS/CUTE kernels tend to keep register usage lower (~152 regs in our
//   measurements). One simple way to reduce registers is to **load B right
//   before it is used** (still 4 ldmatrix loads per kgroup, but without storing
//   the full 2-kgroup B tile persistently in registers).
//
// Design:
// - Keep A fragments buffered (`aReg[2][4][4]`) so we still reuse A across N.
// - Load B fragments on-the-fly inside the `nb` loop for each kgroup.
// - Keep the rest (cp.async pipeline, stage pointers, grid swap) identical to v52.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_streamB(
    ...
    static_assert(N_STAGES == 3, "v54 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // Double-buffered A fragments: slot 0 = kgroup0, slot 1 = kgroup1.
    uint32_t aReg[2][4][4];

    // Initial load: A kgroup0 (kk=0) into slot 0 from stage 0.
#pragma unroll
    for (int m = 0; m < 4; ++m) {
    ...
    for (int k_iter = 0; k_iter < MAIN_ITERS; ++k_iter) {
        ...
        // Load A kgroup1 (kk=16) into slot 1.
#pragma unroll
        for (int m = 0; m < 4; ++m) {
        ...
        // MMA: kgroup0 (kk=0). Stream B fragments inside the nb loop.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t b0, b1, b2, b3;
            uint32_t const addr_b = b_load_base_k + b_ld_off_bytes[0][nb];
...
#pragma unroll
        ...
        // MMA: kgroup1 (kk=16). Stream B fragments inside the nb loop.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t b0, b1, b2, b3;
...
#pragma unroll
        ...
        // Load next iteration's A kgroup0 (kk=0) into slot 0 (new stage).
#pragma unroll
        for (int m = 0; m < 4; ++m) {
            uint32_t const addr_a = a_load_base + a_ld_off_bytes[0][m];
    ...
    // Tail and final iteration: keep the same streaming-B strategy.
    {
        // Iteration k_iter = MAIN_ITERS (e.g., 126): compute, then drain last stage.
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;

        // Load A kgroup1 into slot 1.
#pragma unroll
        for (int m = 0; m < 4; ++m) {
            uint32_t const addr_a = a_load_base_k + a_ld_off_bytes[1][m];
            ldmatrix_x4(aReg[1][m][0], aReg[1][m][1], aReg[1][m][2],
...
#pragma unroll
        ...
        // Advance stage pointers to the last stage and load next iter's A kgroup0.
        a_load_base += A_STAGE_BYTES;
        b_load_base += B_STAGE_BYTES;
        a_store_base += A_STAGE_BYTES;
        b_store_base += B_STAGE_BYTES;
