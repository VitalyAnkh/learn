...
// v55: warp-staggered cp.async schedule (teaching kernel).
//
// Motivation:
// - Our current best (v52) still issues all 6 `cp.async` instructions as a
//   tight burst between kgroup0 and kgroup1. Because all warps execute the
//   same schedule, this creates short windows where *no warp* issues `HMMA`,
//   which shows up as ~92–93% tensor-pipe active (≈180 TFLOPS vs CUTLASS ~197).
// - CUTLASS/CUTE SASS shows `LDGSTS` (cp.async) interleaved with `HMMA`. One
//   reason this can help is that it lets different warp schedulers issue copy
//   (MIO) and MMA (tensor) in the same cycles when warps are not perfectly
//   lockstep at the copy points.
//
// Design:
// - Same math tile, same shared-memory layout, same postwait trick as v52.
// - Only changes the **kgroup0 schedule**:
//   - Split the 32 HMMA ops into 4 `nb` blocks (each has 8 HMMA ops).
//   - Insert a staggered `cp.async` schedule at the `nb` boundaries:
//       even warps issue some copies earlier; odd warps issue them one block
//       later. This forces warp-level phase differences (within the CTA) at
//       the copy issue points.
// - Commit group happens at the same logical point for all warps (end of kgroup0).
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_warpstagger_nb(
    ...
    static_assert(N_STAGES == 3, "v55 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    bool const warp_even = (warp_id & 1) == 0;
...
#pragma unroll
        ...
        // Precompute next-tile gmem pointers for lower overhead in the HMMA stream.
        ...
        // -------------------------
        // MMA: kgroup0 (kk=0) using slot 0, split into nb blocks.
        // -------------------------
        {
            // nb = 0
            {
...
#pragma unroll
                for (int m = 0; m < 4; ++m) {
                    ...
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][0][0], c[m][0][1], a0, a1, a2, a3, b0, b1);
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][1][0], c[m][1][1], a0, a1, a2, a3, b2, b3);
                }
            }

            // First copy window: even warps issue early (odd warps delay by 1 nb).
            if (warp_even) {
...
#pragma unroll
                for (int m = 0; m < 4; ++m) {
                    ...
                    uint32_t const a3 = aReg[0][m][3];
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][2][0], c[m][2][1], a0, a1, a2, a3, b0, b1);
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][3][0], c[m][3][1], a0, a1, a2, a3, b2, b3);
                }
            }

            // Second copy window: even warps issue A0; odd warps catch up on B0/B1.
            if (warp_even) {
...
#pragma unroll
                for (int m = 0; m < 4; ++m) {
                    ...
                    uint32_t const a3 = aReg[0][m][3];
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][4][0], c[m][4][1], a0, a1, a2, a3, b0, b1);
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][5][0], c[m][5][1], a0, a1, a2, a3, b2, b3);
                }
            }

            // Third copy window: even warps issue B2/B3; odd warps issue A0.
            if (warp_even) {
...
#pragma unroll
                for (int m = 0; m < 4; ++m) {
                    ...
                    uint32_t const a3 = aReg[0][m][3];
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][6][0], c[m][6][1], a0, a1, a2, a3, b0, b1);
                    mma_sync_m16n8k16_row_col_f16(
                        c[m][7][0], c[m][7][1], a0, a1, a2, a3, b2, b3);
                }
            }

            // Final copy window: everyone must issue A1; odd warps also issue B2/B3.
            if (!warp_even) {
                cp_async_cg_L2_16B_addr(b_s2, gB_pref2);
                cp_async_cg_L2_16B_addr(b_s3, gB_pref3);
            }
            ...
            // All warps commit exactly once per K-iter.
            cp_async_commit_group();
        }
...
#pragma unroll
    ...
    // Tail: reuse v52 logic to keep v55 focused on the steady-state schedule.
    // We intentionally keep the tail identical to v52 to isolate perf effects.
    {
        // Iteration k_iter = MAIN_ITERS: compute, then drain last stage.
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;
...
#pragma unroll
        ...
        // Advance stage pointers and load next iter's kgroup0.
        a_load_base += A_STAGE_BYTES;
        b_load_base += B_STAGE_BYTES;
        a_store_base += A_STAGE_BYTES;
        b_store_base += B_STAGE_BYTES;
