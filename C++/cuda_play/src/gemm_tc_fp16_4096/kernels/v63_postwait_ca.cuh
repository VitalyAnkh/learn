...
// v63: v52 + `cp.async.ca` (CUTLASS-style cache operator) for the gmem->smem
// pipeline.
//
// Motivation:
// - Nsight Compute shows a large gap in `Stall LG Throttle` between our v52 and
//   the CUTLASS reference kernel for the same tile shape.
// - CUTLASS commonly uses `cp.async.ca.shared.global[.L2::128B]` rather than
//   `.cg` on SM80+, which can reduce the latency/pressure of async copies on
//   some GPUs.
//
// Design:
// - Identical to v52 (same TB/Warp tile, same shared layout, same schedule).
// - Only changes the async copy instruction from `.cg` to `.ca`.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_ca(
    ...
    static_assert(N_STAGES == 3, "v63 is specialized for N_STAGES=3");
...
#pragma unroll
        ...
        // Load kgroup0 for the final iteration.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t const addr_b = b_load_base + b_ld_off_bytes[0][nb];
            ldmatrix_x4_trans(bReg[0][nb][0], bReg[0][nb][1], bReg[0][nb][2],
...
#pragma unroll
    ...
    // Final iteration (k_iter = MAIN_ITERS+1): only compute, no more prefetch.
    {
        uint32_t const a_load_base_k = a_load_base;
        uint32_t const b_load_base_k = b_load_base;
