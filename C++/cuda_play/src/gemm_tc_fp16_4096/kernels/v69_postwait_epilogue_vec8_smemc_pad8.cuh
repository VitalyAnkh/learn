...
// v69: v67 + pad the C scratch leading dimension by +8 half.
//
// Motivation:
// - v67 uses a CTA-wide row-major scratch tile (128x256) and then performs
//   vectorized shared loads (`uint4`) for the global store phase.
// - Nsight Compute shows our kernel has much higher `Stall MIO Throttle` than
//   the CUTLASS reference. One plausible contributor is shared-memory bank
//   conflicts on those wide shared loads (128b).
// - Padding the scratch row stride by +8 half (=16 bytes) changes bank mapping
//   and can reduce conflicts for 128b accesses while still fitting in the
//   existing 72KB shared allocation (As+Bs).
//
// Design:
// - Same mainloop as v67.
// - Epilogue scratch uses LD = 256 + 8.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_epilogue_vec8_smemc_pad8(
    ...
    static_assert(N_STAGES == 3, "v69 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // Epilogue: padded scratch (LD=264) + vec8 global stores.
    __syncthreads();
