...
// v76: v70 + `__maxnreg__(160)` (attempt to match CUTLASS register footprint).
//
// Motivation:
// - CUTLASS reference kernel uses ~152 registers/thread (measured via cuobjdump)
//   while our best kernels are ~168 regs/thread.
// - Even when occupancy is capped by shared memory (1 CTA/SM), a lower register
//   footprint can reduce pressure on the register file / scheduler and can help
//   sustain higher boost clocks (power-limited cases).
// - This variant forces a tighter register allocation target. If it does *not*
//   introduce spills, it may improve performance; if it spills, performance will
//   regress and we keep it as a cautionary example.
//
// Notes:
// - Mainloop and epilogue are identical to v70.
// - The only change is the per-kernel register cap.
template <int N_STAGES>
__global__ __maxnreg__(160) void
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_maxnreg160(
    ...
    static_assert(N_STAGES == 3, "v76 is specialized for N_STAGES=3");
