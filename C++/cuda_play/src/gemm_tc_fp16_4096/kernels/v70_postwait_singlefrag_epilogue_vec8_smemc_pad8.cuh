...
// v70: v66 + pad the C scratch leading dimension by +8 half.
//
// Motivation:
// - v66 uses a CTA-wide scratch tile in shared memory (reuse As+Bs), then does
//   vectorized global stores (`uint4` = 16B, 8 half).
// - Padding the scratch row-stride by +8 half (=16B) changes bank mapping for
//   the wide shared loads in the epilogue, which can reduce bank conflicts.
// - v69 showed that a padded scratch can help for the non-singlefrag epilogue
//   path; this applies the same idea to the singlefrag mainloop.
//
// Design:
// - Same mainloop as v66 (singlefrag overwrite for kgroup1).
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8(
    ...
    static_assert(N_STAGES == 3, "v70 is specialized for N_STAGES=3");
