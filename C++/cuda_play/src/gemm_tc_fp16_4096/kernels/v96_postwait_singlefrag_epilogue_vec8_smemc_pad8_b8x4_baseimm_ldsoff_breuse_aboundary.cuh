...
// v96: v95 + reverse the `m` order for the second (n1) HMMA loop to encourage
// **A operand reuse across loop boundaries**, while keeping B reuse inside each
// (n0 / n1) segment.
//
// Why this might help (Ada / sm_89, power/boost sensitivity):
// - v83 ordering tends to get `.reuse` on the *second* HMMA operand (often A),
//   but not on the third operand (often B).
// - v95 reorders HMMA to keep the same B operand across consecutive HMMA, and
//   we observe `.reuse` on the *third* operand (B), helping TFLOPS a bit.
// - CUTLASS SASS often shows a *mix*: most HMMA use `.reuse` on B, but some
//   boundary HMMA use `.reuse` on A. This can reduce register-file reads and
//   help boost clocks under `sw_power_cap`.
//
// Technique:
// - For a fixed `nb`, do:
//     for m = 0..3: HMMA(..., B01)  // B reuse
//     for m = 3..0: HMMA(..., B23)  // B reuse + A reuse at the boundary
//   so that A[m=3] is used in two consecutive instructions across the boundary.
// - Across `nb` boundaries, the last instruction uses A[m=0], and the next `nb`
//   starts from m=0, also enabling A reuse at another boundary.
//
// Everything else (tile, pipeline, shared layout, epilogue) matches v95/v83.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_b8x4_baseimm_ldsoff_breuse_aboundary(
    ...
    static_assert(N_STAGES == 3, "v96 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // ------------------------------------------------------------
    // Copy thread-map (same as v83/v95)
    // ------------------------------------------------------------
...
#pragma unroll
    ...
    for (int k_iter = 0; k_iter < MAIN_ITERS; ++k_iter) {
        // ------------------------------------------------------------
        // MMA kgroup0 (A+B reuse boundary-friendly ordering)
        // ------------------------------------------------------------
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
...
#pragma unroll
            ...
            // n1: m desc (3->0) to reuse A at the boundary (m=3) and at nb boundary (m=0)
#pragma unroll
            for (int m = 3; m >= 0; --m) {
                uint32_t const a0 = aReg[m][0];
                uint32_t const a1 = aReg[m][1];
...
#pragma unroll
    ...
    // Tail + epilogue: reuse v95 structure, but keep the same ordering.
...
#pragma unroll
    ...
    // Final iteration (no more prefetch) + epilogue: identical to v83/v95.
    // NOTE: Epilogue helpers live in the main TU; we reuse them unchanged.
    {
        // Final kgroup0 HMMA
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
...
#pragma unroll
    ...
    // ------------------------------------------------------------
    // Epilogue: scratch + vec8 stores (same as v83/v95)
    // ------------------------------------------------------------
    __syncthreads();
