...
//
// Teaching notes:
// - "base + immediate" is a micro-optimization that helps when the kernel is
//   already tensor-core heavy and the remaining gap is integer/MIO overhead.
// - How to tell this worked:
//   - In SASS, look for `LDGSTS.* [Rbase+0x...]` patterns (immediate offsets),
//     instead of sequences that compute a new pointer register for each copy.
// - The immediates used here come from simple geometry:
//   - A: +64 rows in a 128x32 tile corresponds to +4096B (`0x1000`) in shared.
//   - B: +64 half elements corresponds to +128B (`0x80`) in shared and global.
