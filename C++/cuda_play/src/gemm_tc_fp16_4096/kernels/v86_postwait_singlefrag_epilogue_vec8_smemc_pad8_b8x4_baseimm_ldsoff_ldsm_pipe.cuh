...
// v86: v83 + pipeline the B-side `ldmatrix` for kgroup1 into the kgroup0 MMA loop.
//
// Background:
// - CUTLASS/CUTE (~196–198 TFLOPS on this machine) interleaves `LDSM` (ldmatrix)
//   very early and very tightly with `HMMA` in the steady-state loop.
// - Our v83 already matches many CUTLASS tactics (base+imm `LDGSTS`, kgroup1
//   loads via +0x2000, and a CUTLASS-style epilogue), but its source schedule
//   still does a burst:
//     * compute all kgroup0 HMMA
//     * then load all kgroup1 fragments (4x B + 4x A ldmatrix)
//   which can shrink the useful `LDSM -> HMMA` dependency distance and trigger
//   more scoreboard stalls.
//
// Key idea in this variant:
// - Keep the same register footprint by *overwriting* `bReg[nb]` once it's no
//   longer needed for kgroup0:
//     * compute nb=0 of kgroup0 (uses bReg[0])
//     * overwrite bReg[0] with kgroup1's B fragment for nb=0
//     * compute nb=1..3 of kgroup0 (independent of bReg[0])
//   This lets ptxas schedule those future `LDSM` instructions in-between `HMMA`,
//   closer to CUTLASS' warp-level software pipeline, without needing a second
//   B fragment buffer (which would likely increase registers).
//
// Expected effect:
// - More CUTLASS-like `LDSM/HMMA` interleave in SASS steady-state.
// - Potentially higher stable TFLOPS (goal: close the remaining ~3–5% gap).
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_b8x4_baseimm_ldsoff_ldsm_pipe(
    ...
    static_assert(N_STAGES == 3, "v86 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // ------------------------------------------------------------
    // ldmatrix offsets (same as v83)
    // ------------------------------------------------------------
    constexpr uint32_t A_M_DELTA_BYTES = 0x400; // +16 rows in A (layout-specific)
...
#pragma unroll
    ...
    // For A, just compute the base for (m=0) at kk=0 and kk=16.
    {
        int const a_r = warp_row * 64 + (lane_id & 15);
        int const a_c0 = ((lane_id >> 4) * 8);     // kk=0
        int const a_c1 = 16 + ((lane_id >> 4) * 8); // kk=16
        a_ld_base_bytes[0] =
            static_cast<uint32_t>(tensorop_rm_offset<32, TILE_K>(a_r, a_c0) << 1);
        a_ld_base_bytes[1] =
            static_cast<uint32_t>(tensorop_rm_offset<32, TILE_K>(a_r, a_c1) << 1);
    }
...
#pragma unroll
    for (int stage = 0; stage < (N_STAGES - 1); ++stage) {
        ...
        cp_async_cg_L2_16B_addr(a_store, gA_base + a_row0 * kK + a_col);
        cp_async_cg_L2_16B_addr(a_store + A_SMEM_ROW_DELTA_BYTES,
                                gA_base + (a_row0 * kK + a_col + A_ROW_DELTA));

        __half const *gB0 = gB_base + b_row * kN + b_col0;
        cp_async_cg_L2_16B_addr(b_store, gB0);
    ...
    // Running gmem base pointers (half element pointers).
    __half const *gA_pref =
        A + block_m * kK + (N_STAGES - 1) * TILE_K + a_row0 * kK + a_col;
    __half const *gA_pref2 = gA_pref + A_ROW_DELTA;
    __half const *gB_pref =
        B + (N_STAGES - 1) * TILE_K * kN + block_n + b_row * kN + b_col0;
...
#pragma unroll
    ...
    for (int k_iter = 0; k_iter < MAIN_ITERS; ++k_iter) {
        // ------------------------------------------------------------
        // MMA kgroup0 (kk=0) + pipeline B loads for kgroup1 (kk=16)
        // ------------------------------------------------------------
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
...
#pragma unroll
            ...
            // After consuming kgroup0's bReg[nb], overwrite it with kgroup1's
            // B fragment. This load is independent of the remaining nb blocks
            // and can be scheduled between their HMMA instructions.
            uint32_t const addr_b = b_load_base + b_ld_off_bytes[nb];
        ...
        // Load kgroup1 A (kk=16). B is already in bReg from the pipelined loads.
        {
...
#pragma unroll
    ...
    // Tail + epilogue: reuse v83 implementation (copy/paste) by calling its
    // existing helper: we inline the exact same code by including v83's tail.
    //
    // For simplicity and minimal risk, keep the tail identical to v83. The
    // steady-state loop above is the only intended change in v86.
    //
    // NOTE: We cannot literally "call into v83" without duplicating code, so
    // below is the v83 tail+epilogue block with no semantic changes.

    // ---------------------------
    // Tail + epilogue (from v83)
    // ---------------------------
    {
        // k_iter = MAIN_ITERS
#pragma unroll
