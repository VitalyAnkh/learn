...
// v52: v49 but delays `cp.async.wait_group + __syncthreads()` until *after*
// finishing kgroup1 MMA.
//
// Motivation:
// - In v49, we issue prefetch cp.async, then immediately `wait_group`+barrier,
//   and only then execute kgroup1 MMA.
// - But kgroup1 MMA uses registers (`aReg[1]/bReg[1]`) and does not touch the
//   stage being prefetched. That means we can safely do useful work while the
//   next stage is still in flight.
// - Delaying the wait/barrier increases overlap of gmem->smem latency with
//   compute and may reduce barrier-related bubbles at 1 CTA/SM occupancy.
//
// Design:
// - Same TB/Warp tile, same shared layout, same A/B packing, same B-reuse order.
// - Only changes the **mainloop schedule**:
//   - kgroup0 MMA
//   - issue cp.async prefetch + commit
//   - kgroup1 MMA (overlaps with cp.async)
//   - wait_group + barrier
//   - advance stage pointers and load next-iter kgroup0
//
// Teaching notes:
// - `cp.async` is a *producer* of the next stage in shared memory. The consumers
//   are the `ldmatrix` loads that feed `mma.sync`.
// - The key correctness rule for overlap is:
//     "Don't `ldmatrix` from a stage until you've waited for the async copies
//      that populate that stage."
//   In this kernel, kgroup1 uses data already resident in registers
//   (`aReg[1]`/`bReg[1]`), so it is safe to delay `cp.async.wait_group` until
//   after kgroup1 HMMA.
// - On Ada, these scheduling tweaks often matter more when occupancy is 1 CTA/SM
//   (common for large Tensor Core kernels due to registers/shared usage):
//   fewer barrier bubbles => better tensor pipe utilization.
//
// How to verify what this teaches:
// - SASS: `cuobjdump --dump-sass ... | rg "LDGSTS|HMMA|LDSM|DEPBAR|BAR"` should
//   show async copy (`LDGSTS`) in the steady-state loop.
// - Nsight Compute: compare barrier stall metrics before/after this change.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait(
    ...
    static_assert(N_STAGES == 3, "v52 is specialized for N_STAGES=3");
...
#pragma unroll
        ...
        // Only now do we wait/sync for the next stage to become visible.
        cp_async_wait_group<N_STAGES - 2>();
        __syncthreads();
