...
// v72: v70, but uses **dynamic shared memory** (extern __shared__) for A/B/scratch.
//
// Motivation:
// - CUTLASS' 128x256x32x3 kernels commonly use dynamic shared (static shared = 0),
//   which can influence register allocation / address computation / scheduling.
// - This is a focused experiment: keep the mainloop + epilogue identical to v70,
//   only change how shared memory is declared and addressed.
//
// Notes:
// - Host launch must pass dynamic smem = 73728 bytes (3 stages of A+B tiles).
// - We still reuse the same shared region as a CTA-wide scratch tile in the epilogue.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_dynsmem(
    ...
    static_assert(N_STAGES == 3, "v72 is specialized for N_STAGES=3");
    ...
    // Dynamic shared: A stages then B stages.
    extern __shared__ __align__(128) __half smem[];
    __half *As = smem;
    __half *Bs = smem + N_STAGES * A_STAGE_ELEMS;
...
#pragma unroll
    ...
    __half *smem_C = As; // reuse A region (shared is a single slab)
