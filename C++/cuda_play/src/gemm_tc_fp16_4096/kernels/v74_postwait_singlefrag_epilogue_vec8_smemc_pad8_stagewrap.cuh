...
// v74: v70 + stage ring-buffer update via a single stage counter.
//
// Motivation:
// - v70 maintains 4 shared-memory base pointers (A/B load/store) and wraps each
//   with 4 independent `if (ptr == end) ptr = base;` checks in the hot loop.
// - CUTLASS SASS tends to use a small stage index + predicated selects for ring
//   buffer management, keeping the steady-state loop very lean.
// - This variant tries to reduce integer/control overhead by:
//     * advancing all 4 pointers unconditionally, and
//     * using a *single* stage counter to trigger wrap (subtract ring size).
//
// Expected effect:
// - Fewer compares/branches => slightly lower INT pipe pressure / power.
// - Potentially higher sustained clocks and/or slightly tighter HMMA issue.
//
// Notes:
// - Mainloop math, data layout, and epilogue are identical to v70.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_postwait_singlefrag_epilogue_vec8_smemc_pad8_stagewrap(
    ...
    static_assert(N_STAGES == 3, "v74 is specialized for N_STAGES=3");
...
#pragma unroll
        ...
        // Advance ring pointers by one stage.
        a_load_base += A_STAGE_BYTES;
        b_load_base += B_STAGE_BYTES;
        a_store_base += A_STAGE_BYTES;
        b_store_base += B_STAGE_BYTES;

        // Wrap load/store pointers independently, but only compare against the
        // A-end address (B wraps on the same iteration).
...
#pragma unroll
    ...
    // Tail: identical to v70/v66 (keep for clarity).
    {
