...
// v90: combine v88 (midwait + next-iter fragment pipeline) with v89 (cp.async
// interleaving during kgroup0 HMMA).
//
// Motivation:
// - v88 alone didn't improve throughput, likely because `LDGSTS` (cp.async)
//   issue is still a burst and the stage-advance barrier placement alone doesn't
//   sufficiently change steady-state overlap.
// - v89 interleaves `cp.async` issue points, but keeps the postwait barrier,
//   so it still can't overlap "next-iter LDSM" with kgroup1 HMMA.
//
// Technique:
// - Within each TILE_K=32 iteration:
//   1) Prefetch kgroup1 fragments into *_1
//   2) Compute kgroup0 HMMA using *_0 while interleaving the 6 cp.async issues
//   3) commit_group
//   4) wait_group + __syncthreads + stage advance (midwait, CUTLASS-like)
//   5) Load next iteration's kgroup0 into *_0
//   6) Compute kgroup1 HMMA using *_1 (while next-iter LDSM is in flight)
//
// Goal:
// - Move closer to CUTLASS' mac-loop rhythm: fewer bursty copy regions, earlier
//   stage advance, and earlier next-iter fragment loads.
...
gemm_tc_mma_cutlasslike_tn_cpasync_fp16acc_128x256x32_warp2x4_cutlass_style_faststage_noif_breuse_swapxy_midwait_cpinterleave_fragpipe(
    ...
    static_assert(N_STAGES == 3, "v90 is specialized for N_STAGES=3");
...
#pragma unroll
    ...
    // ------------------------------------------------------------
    // Copy thread-map (same as v83/v88/v89)
    // ------------------------------------------------------------
...
#pragma unroll
    ...
    // Initial load: kgroup0 (kk=0) into *_0
#pragma unroll
    for (int nb = 0; nb < 4; ++nb) {
...
#pragma unroll
        ...
        // kgroup0 HMMA with cp.async interleave (same pattern as v89).
        {
            // nb=0
            {
                uint32_t const b0 = bReg0[0][0];
                uint32_t const b1 = bReg0[0][1];
                uint32_t const b2 = bReg0[0][2];
                uint32_t const b3 = bReg0[0][3];
                int const n0 = 0;
                int const n1 = 1;
#pragma unroll
                for (int m = 0; m < 4; ++m) {
            ...
            // nb=1
            {
                uint32_t const b0 = bReg0[1][0];
                uint32_t const b1 = bReg0[1][1];
                uint32_t const b2 = bReg0[1][2];
                uint32_t const b3 = bReg0[1][3];
                int const n0 = 2;
                int const n1 = 3;
#pragma unroll
                for (int m = 0; m < 4; ++m) {
            ...
            // nb=2
            {
                uint32_t const b0 = bReg0[2][0];
                uint32_t const b1 = bReg0[2][1];
                uint32_t const b2 = bReg0[2][2];
                uint32_t const b3 = bReg0[2][3];
                int const n0 = 4;
                int const n1 = 5;
#pragma unroll
                for (int m = 0; m < 4; ++m) {
            ...
            // nb=3
            {
                uint32_t const b0 = bReg0[3][0];
                uint32_t const b1 = bReg0[3][1];
                uint32_t const b2 = bReg0[3][2];
                uint32_t const b3 = bReg0[3][3];
                int const n0 = 6;
                int const n1 = 7;
#pragma unroll
                for (int m = 0; m < 4; ++m) {
        ...
        // Load next iteration kgroup0 into *_0.
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t const addr_b = b_load_base + b_ld_off_bytes[nb];
            ldmatrix_x4_trans(bReg0[nb][0], bReg0[nb][1], bReg0[nb][2], bReg0[nb][3],
...
#pragma unroll
    ...
    // Tail handling + epilogue: re-use v88 tail+epilogue pattern (compute last
    // 2 blocks with a final drain). For brevity, we keep it identical to v88.
    //
    // We include v88-style tail by directly inlining its logic here.
...
#pragma unroll
        ...
        // Load block127 kgroup0 into *_0 (overlap with kgroup1 of block126).
#pragma unroll
        for (int nb = 0; nb < 4; ++nb) {
            uint32_t const addr_b = b_load_base + b_ld_off_bytes[nb];
            ldmatrix_x4_trans(bReg0[nb][0], bReg0[nb][1], bReg0[nb][2], bReg0[nb][3],
...
#pragma unroll
    ...
    constexpr int SMEM_C_LD = TILE_N + SMEM_C_PAD;
...
#pragma unroll
    ...
    constexpr int VEC_ITERS = VEC_COUNT / THREADS_PER_BLOCK;
